...
export function NewBookingForm() {
  ...
  return (
    ...
    <form onSubmit={handleSubmit} className="mt-8 rounded-[2rem] bg-white p-8 shadow-sm">
      <div className="grid gap-5 md:grid-cols-2">
        <div>
          <label className="text-sm font-semibold text-[#1F5E4B]">Guest Name</label>
          <div className="mt-2 flex items-center gap-3 rounded-2xl border p-4">
            <User className="h-5 w-5 text-[#C9A227]" />
            <input name="guestName" value={booking.guestName} onChange={handleChange} className="w-full outline-none" placeholder="Enter guest name" />
          </div>
        </div>
        ...
        <div>
          ...
          <div className="mt-2 flex items-center gap-3 rounded-2xl border p-4">
            ...
            <input name="phone" value={booking.phone} onChange={handleChange} className="w-full outline-none" placeholder="Enter phone number" />
          </div>
        </div>
        ...
        <div>
          ...
          <div className="mt-2 flex items-center gap-3 rounded-2xl border p-4">
            ...
            <input name="email" value={booking.email} onChange={handleChange} className="w-full outline-none" placeholder="Enter email address" />
          </div>
        </div>
        ...
        <div>
          ...
          <div className="mt-2 flex items-center gap-3 rounded-2xl border p-4">
            ...
            <input type="number" min="1" name="guests" value={booking.guests} onChange={handleChange} className="w-full outline-none" />
          </div>
        </div>

        <div>
          <label className="text-sm font-semibold text-[#1F5E4B]">Booking Source</label>
          <select name="source" value={booking.source} onChange={handleChange} className="mt-2 w-full rounded-2xl border p-4 outline-none">
        ...
        <div>
          <label className="text-sm font-semibold text-[#1F5E4B]">Room Type</label>
          <select name="roomType" value={booking.roomType} onChange={handleChange} className="mt-2 w-full rounded-2xl border p-4 outline-none">
...
<option value="Executive Suite">Executive Suite</option>
  ...
  >
    <option value="">
      {booking.roomType ? "Select room number" : "Select room type first"}
    </option>

    {availableRooms.map((room) => (
      <option key={room.id} value={room.number}>
        Room {room.number}, Floor {room.floor}, {room.price.toLocaleString()} FCFA
      </option>
    ))}
  ...
  <div className="rounded-2xl bg-[#F8F7F3] p-5 md:col-span-2">
    ...
    <div className="mt-4 grid gap-4 sm:grid-cols-4">
      ...
      <div>
        ...
        <div>
          <label className="text-sm font-semibold text-[#1F5E4B]">Check-in</label>
          <div className="mt-2 flex items-center gap-3 rounded-2xl border p-4">
            <CalendarDays className="h-5 w-5 text-[#C9A227]" />
            <input type="date" name="checkIn" value={booking.checkIn} onChange={handleChange} className="w-full outline-none" />
          </div>
        </div>

        <div>
          <label className="text-sm font-semibold text-[#1F5E4B]">Check-out</label>
          <div className="mt-2 flex items-center gap-3 rounded-2xl border p-4">
            <CalendarDays className="h-5 w-5 text-[#C9A227]" />
            <input type="date" name="checkOut" value={booking.checkOut} onChange={handleChange} className="w-full outline-none" />
          </div>
        </div>

        <div className="md:col-span-2">
          <label className="text-sm font-semibold text-[#1F5E4B]">Staff Note</label>
          <textarea name="note" value={booking.note} onChange={handleChange} rows={4} className="mt-2 w-full rounded-2xl border p-4 outline-none" placeholder="Add any note about the booking" />
        </div>
      </div>

      <Button type="submit" className="mt-8 rounded-full bg-[#1F5E4B] px-10 hover:bg-[#063D2E]">
        Create Booking
      </Button>
    </form>
  );
